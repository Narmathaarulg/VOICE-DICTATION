...
def initialize_gemini():
    ...
    model = gemini.GenerativeModel('models/gemini-2.0-flash-exp')  # Updated model
    return model
...
# -------------------- LIVE TRANSCRIPTION STATE (NEW!) --------------------
active_transcriptions = {}

# -------------------- AUDIO RECORD (EXISTING) --------------------
...
def handle_start_live_transcription(data):
    """Start live transcription session (NEW!)"""
    try:
...
def handle_audio_chunk(data):
    """Receive and process audio chunks (NEW!)"""
    try:
...
def handle_stop_live_transcription(data):
    """Stop live transcription and process final results (NEW!)"""
    try:
