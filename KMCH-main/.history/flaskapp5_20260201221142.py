...
def handle_audio_chunk(data):
    ...
    try:
        ...
        # Use Google Speech-to-Text for live transcription
        if session.speech_client:
            try:
                ...
                # Fallback: use Gemini for transcription (slower but works)
                emit('live_transcript', {
            ...
            # Fallback message if Speech client not available
            emit('live_transcript', {
                'transcript': '[Live transcription requires Google Speech-to-Text setup]',
                'confidence': 0,
                'is_final': False
            })
