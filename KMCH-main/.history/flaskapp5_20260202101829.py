...
def initialize_gemini():
    ...
    # Use gemini-2.5-flash (confirmed working by user)
    try:
        model = gemini.GenerativeModel('models/gemini-2.5-flash')
        print(f"✅ Successfully initialized Gemini model: gemini-2.5-flash")
        return model
...
def initialize_deepgram():
    """Initialize Deepgram client for SDK v3"""
    try:
        ...
        # SDK v3 uses DeepgramClient instead of Deepgram
        from deepgram import DeepgramClient, PrerecordedOptions
        
        dg_client = DeepgramClient(deepgram_api_key)
        print("✅ Deepgram initialized successfully (SDK v3)")
        return dg_client
    except Exception as e:
...
async def process_deepgram_stream(session_id, audio_data):
    """Process audio through Deepgram SDK v3"""
    try:
        session = live_sessions.get(session_id)
        if not session or not session.deepgram_client:
            return
        
        # SDK v3 - Use Listen API with prerecorded
        ...
        # Get transcription using SDK v3 listen.rest.v() method
        response = session.deepgram_client.listen.rest.v("1").transcribe_file(
            payload, 
            options
        )
